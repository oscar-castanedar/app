...
router.post("/signup", async (req, res) => {
    ...
    if(!emailExist){
        ...
        return res.redirect('/?error=' + encodeURIComponent('Este correo ya existe!'));
    }
});
...
router.post("/login", async (req, res) => {
    ...
    if(!user.email) return res.redirect('/?error=' + encodeURIComponent('Este correo no existe'));

    const validPass = await bcrypt.compare(req.body.Password, user.password);
    if(!validPass) return res.redirect('/?error=' + encodeURIComponent('Contraseña invalida'));
...
router.post("/restlogin", async (req, res) => {
    ...
    if(!user.email) return res.status(400).send('Este correo no existe');
